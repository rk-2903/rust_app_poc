...
impl TensorData {
    ...
    fn into_vec_unchecked<E: Element>(self) -> Result<Vec<E>, DataError> {
        ...
        //
        // Locally patched (see app/vendor/README.md in the consuming project): upstream's
        // fallback here retried `bytemuck::checked::try_cast_slice` on the very same
        // (misaligned) byte buffer, which fails identically since the cast target's
        // alignment requirement is a property of the source pointer, not of the cast
        // call itself. This allocates a freshly (and therefore correctly) aligned `Vec<E>`
        // and copies into it instead.
        let src = me.bytes.read(Reader::new())?;
        let elem_size = core::mem::size_of::<E>();
        Ok(src
            .chunks_exact(elem_size)
            .map(bytemuck::checked::pod_read_unaligned::<E>)
            .collect())
    }
